...
namespace Discount.Grpc.Services
{
    public class DiscountService(DiscountContext dbContext, ILogger<DiscountService> logger) : DiscountProtoService.DiscountProtoServiceBase
    {
        ...
        public override async Task<CouponModel> UpdateDiscount(UpdateDiscountRequest request, ServerCallContext context)
        {
            ...
            var existingCoupon = await dbContext.Coupons.FirstOrDefaultAsync(x => x.Id == request.Coupon.Id);
            if (existingCoupon is null)
                throw new RpcException(new Status(StatusCode.NotFound, "Coupon not found for update."));

            // Update properties of the existing coupon
